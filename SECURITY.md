...
Compose includes a non-root user, read-only root filesystem, dropped capabilities, a named data volume, PID and memory limits, and loopback exposure. Those runtime controls need verification on your Docker host. The supplied Docker smoke test exercises deployment and checkpoint recovery. The base-image tag follows Node 26 security updates rather than pinning a digest; pin and scan a known image digest for controlled deployments.
